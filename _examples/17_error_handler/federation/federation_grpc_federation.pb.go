...
func (s *FederationService) resolve_Org_Federation_LocalizedMessage(ctx context.Context, req *Org_Federation_LocalizedMessageArgument) (*LocalizedMessage, error) {
	...
	value := &localValueType{LocalValue: grpcfed.NewLocalValue(ctx, s.celTypeHelper, s.envOpts, s.celPlugins, "grpc.federation.private.LocalizedMessageArgument", req)}
	defer func() {
		if err := value.Close(ctx); err != nil {
			s.logger.ErrorContext(ctx, err.Error())
		}
								...
								if err := grpcfed.EvalDef(ctx, value, grpcfed.Def[*CustomMessage, *localValueType]{
									...
									Message: func(ctx context.Context, value *localValueType) (any, error) {
										...
										if err := grpcfed.SetCELValue(ctx, &grpcfed.SetCELValueParam[*grpcfedcel.Error]{
											...
											UseContextLibrary: false,
											CacheIndex:        11,
											Setter: func(v *grpcfedcel.Error) {
												args.ErrorInfo = v
											},
										}); err != nil {
											return nil, err
