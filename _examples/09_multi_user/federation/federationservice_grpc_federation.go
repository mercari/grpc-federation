...
// Org_Federation_FooArgument is argument for "org.federation.Foo" message.
type Org_Federation_FooArgument struct {
	Client *FederationServiceDependencyServiceClient
}
...
// resolve_Org_Federation_Foo resolve "org.federation.Foo" message.
func (s *FederationService) resolve_Org_Federation_Foo(ctx context.Context, req *Org_Federation_FooArgument) (*Foo, error) {
	s.logger.DebugContext(ctx, "resolve  org.federation.Foo", slog.Any("message_args", s.logvalue_Org_Federation_FooArgument(req)))
	var (
	...
	*/
	...
	s.logger.DebugContext(ctx, "resolved org.federation.Foo", slog.Any("org.federation.Foo", s.logvalue_Org_Federation_Foo(ret)))
	return ret, nil
}
...
func (s *FederationService) resolve_Org_Federation_GetResponse(ctx context.Context, req *Org_Federation_GetResponseArgument) (*GetResponse, error) {
	...
	*/
	...
	s.goWithRecover(eg, func() (interface{}, error) {
		...
		/*
		   {
		     name: "_org_federation_Foo"
		     message: "Foo"
		   }
		*/
		if _, err, _ := sg.Do("_org_federation_Foo_org.federation.Foo", func() (interface{}, error) {
			valueMu.RLock()
			args := &Org_Federation_FooArgument{
				Client: s.client,
			}
			valueMu.RUnlock()
			return s.resolve_Org_Federation_Foo(ctx, args)
		}); err != nil {
			return nil, err
		}
		valueMu.Lock()
		...
		*/
		resUserIface, err, _ := sg.Do("user2_org.federation.User", func() (interface{}, error) {
			...
			args := &Org_Federation_UserArgument{
				Client: s.client,
				UserId: "2", // { name: "user_id", string: "2" }
			}
			valueMu.RUnlock()
	...
	s.goWithRecover(eg, func() (interface{}, error) {
		...
		*/
		resUserIface, err, _ := sg.Do("user_org.federation.User", func() (interface{}, error) {
			...
			args := &Org_Federation_UserArgument{
				Client: s.client,
				UserId: "1", // { name: "user_id", string: "1" }
			}
			valueMu.RUnlock()
			return s.resolve_Org_Federation_User(ctx, args)
		})
...
func (s *FederationService) logvalue_Org_Federation_Foo(v *Foo) slog.Value {
	if v == nil {
...
func (s *FederationService) logvalue_Org_Federation_FooArgument(v *Org_Federation_FooArgument) slog.Value {
	if v == nil {
		return slog.GroupValue()
	}
	return slog.GroupValue()
