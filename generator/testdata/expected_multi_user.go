...
func (s *FederationService) resolve_Org_Federation_GetResponse(ctx context.Context, req *Org_Federation_GetResponseArgument[*FederationServiceDependentClientSet]) (*GetResponse, error) {
	...
	*/
		...
		*/
		{
			valueIface, err, _ := sg.Do("user", func() (any, error) {
				...
				{
					value, err := grpcfed.EvalCEL(ctx1, s.env, "uid.value", envOpts, evalValues, reflect.TypeOf(""))
					if err != nil {
						valueMu.RUnlock()
	...
	{
		value, err := grpcfed.EvalCEL(ctx, s.env, "user2", envOpts, evalValues, reflect.TypeOf((*User)(nil)))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
		...
		*/
		{
			valueIface, err, _ := sg.Do("res", func() (any, error) {
				...
				// { field: "id", by: "$.user_id" }
				{
					value, err := grpcfed.EvalCEL(ctx1, s.env, "$.user_id", envOpts, evalValues, reflect.TypeOf(""))
					if err != nil {
						valueMu.RUnlock()
						grpcfed.RecordErrorToSpan(ctx1, err)
						return nil, err
		...
		*/
		{
			valueIface, err, _ := sg.Do("user", func() (any, error) {
				...
				valueMu.RUnlock()
				valueMu.RLock()
				value, err := grpcfed.EvalCEL(ctx1, s.env, "res.user", envOpts, evalValues, reflect.TypeOf((*user.User)(nil)))
				valueMu.RUnlock()
				return value, err
			})
			if err != nil {
