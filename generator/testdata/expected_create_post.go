...
func (s *FederationService) resolve_Org_Federation_CreatePost(ctx context.Context, req *Org_Federation_CreatePostArgument[*FederationServiceDependentClientSet]) (*CreatePost, error) {
	...
	{
		value, err := grpcfed.EvalCEL(ctx, s.env, "$.title", envOpts, evalValues, reflect.TypeOf(""))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
	...
	{
		value, err := grpcfed.EvalCEL(ctx, s.env, "$.content", envOpts, evalValues, reflect.TypeOf(""))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
...
func (s *FederationService) resolve_Org_Federation_CreatePostResponse(ctx context.Context, req *Org_Federation_CreatePostResponseArgument[*FederationServiceDependentClientSet]) (*CreatePostResponse, error) {
	...
	*/
	{
		valueIface, err, _ := sg.Do("res", func() (any, error) {
			...
			// { field: "post", by: "cp" }
			{
				value, err := grpcfed.EvalCEL(ctx, s.env, "cp", envOpts, evalValues, reflect.TypeOf((*CreatePost)(nil)))
				if err != nil {
					valueMu.RUnlock()
					grpcfed.RecordErrorToSpan(ctx, err)
					return nil, err
	...
	*/
	...
	{
		value, err := grpcfed.EvalCEL(ctx, s.env, "p", envOpts, evalValues, reflect.TypeOf((*post.Post)(nil)))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
