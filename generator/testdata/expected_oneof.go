...
func (s *FederationService) resolve_Org_Federation_GetResponse(ctx context.Context, req *Org_Federation_GetResponseArgument[*FederationServiceDependentClientSet]) (*GetResponse, error) {
	...
	*/
	...
	{
		value, err := grpcfed.EvalCEL(ctx, s.env, "sel.user", envOpts, evalValues, reflect.TypeOf((*User)(nil)))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
...
func (s *FederationService) resolve_Org_Federation_UserSelection(ctx context.Context, req *Org_Federation_UserSelectionArgument[*FederationServiceDependentClientSet]) (*UserSelection, error) {
	...
	*/
	...
	oneof_UserA, err := grpcfed.EvalCEL(ctx, s.env, "m.value == $.value", envOpts, evalValues, reflect.TypeOf(true))
	if err != nil {
		grpcfed.RecordErrorToSpan(ctx, err)
		return nil, err
	}
	oneof_UserB, err := grpcfed.EvalCEL(ctx, s.env, "m.value != $.value", envOpts, evalValues, reflect.TypeOf(true))
	if err != nil {
		grpcfed.RecordErrorToSpan(ctx, err)
		return nil, err
	}
	...
	case oneof_UserA.(bool):
		...
		*/
		{
			valueIface, err, _ := sg.Do("ua", func() (any, error) {
				valueMu.RLock()
		...
		value, err := grpcfed.EvalCEL(ctx, s.env, "ua", envOpts, evalValues, nil)
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
	...
	case oneof_UserB.(bool):
		...
		*/
		{
			valueIface, err, _ := sg.Do("ub", func() (any, error) {
				valueMu.RLock()
				args := &Org_Federation_UserArgument[*FederationServiceDependentClientSet]{
					Client: s.client,
		...
		value, err := grpcfed.EvalCEL(ctx, s.env, "ub", envOpts, evalValues, nil)
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
	...
	default:
		...
		*/
		{
			valueIface, err, _ := sg.Do("uc", func() (any, error) {
				valueMu.RLock()
				args := &Org_Federation_UserArgument[*FederationServiceDependentClientSet]{
					Client: s.client,
				}
				...
				{
					value, err := grpcfed.EvalCEL(ctx, s.env, "$.value", envOpts, evalValues, reflect.TypeOf(""))
					if err != nil {
						valueMu.RUnlock()
						grpcfed.RecordErrorToSpan(ctx, err)
						return nil, err
		...
		value, err := grpcfed.EvalCEL(ctx, s.env, "uc", envOpts, evalValues, nil)
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
