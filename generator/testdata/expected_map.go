...
func (s *FederationService) resolve_Org_Federation_Posts(ctx context.Context, req *Org_Federation_PostsArgument[*FederationServiceDependentClientSet]) (*Posts, error) {
	...
	*/
	{
		valueIface, err, _ := sg.Do("posts", func() (any, error) {
			valueMu.RLock()
			valueMu.RUnlock()
			return grpcfed.EvalCEL(s.env, "res.posts", envOpts, evalValues, reflect.TypeOf([]*post.Post(nil)))
		})
		if err != nil {
			return nil, err
		}
	...
	*/
	...
	// field binding section.
	// (grpc.federation.field).by = "posts.map(e, e.id)"
	{
		value, err := grpcfed.EvalCEL(s.env, "posts.map(e, e.id)", envOpts, evalValues, reflect.TypeOf([]string(nil)))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
	...
	// (grpc.federation.field).by = "posts.map(e, e.title)"
	{
		value, err := grpcfed.EvalCEL(s.env, "posts.map(e, e.title)", envOpts, evalValues, reflect.TypeOf([]string(nil)))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
	...
	// (grpc.federation.field).by = "posts.map(e, e.content)"
	{
		value, err := grpcfed.EvalCEL(s.env, "posts.map(e, e.content)", envOpts, evalValues, reflect.TypeOf([]string(nil)))
		if err != nil {
			grpcfed.RecordErrorToSpan(ctx, err)
			return nil, err
		}
