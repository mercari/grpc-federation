...
`grpc-federation-generator` is a standalone generator that allows you to use the gRPC Federation code generation functionality without `protoc` plugin.
...
`grpc-federation-generator` is made by Go. It can be installed with the following command.
